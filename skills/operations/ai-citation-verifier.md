...
- [ ] If the tribunal asks at oral argument, hearing, or status conference whether AI tools were used in preparing this filing, the truthful answer is the AI-tool log entry recorded for this filing — the signing attorney has reviewed the log and is prepared to disclose the tools used, the stage at which each tool was used, and the verification record without contradicting any prior representation to the court.
...
- Hard rule — *no AI-as-verifier*. The verification step recorded against any YELLOW or RED item must be performed against a primary legal database (Westlaw, Lexis, Bloomberg Law, Fastcase, CourtListener — which as of May 12, 2026 provides free MCP-accessible API access to case law, PACER data, citation analysis, oral argument transcripts, and judge data for any account holder and is the recommended no-cost primary-database option for pro bono, legal-aid, and solo-practice deployments — the official reporter, the issuing court's docket via PACER, or the official publisher of the cited statute or regulation). A second generative AI tool — including a different model, a different vendor's chatbot, or the same model in a fresh session — is *not* a permissible verifier, because it shares the generative pathology of the drafting tool. If the signer's only verification is a confirmation from another AI chatbot, the certification checklist must record the item as *unverified* and the verification queue must be re-run against a primary database before the document is filed. The Eastern District of Pennsylvania's April 27, 2026 second-strike opinion is the anchor authority for this rule and is summarized in `knowledge-base/best-practices/ai-hallucination-sanctions-2026.md`.
- Hard rule — *truthful disclosure on direct judicial inquiry*. If the tribunal asks at oral argument, a hearing, or a show-cause conference whether an AI tool was used in preparing the filing, the signing attorney's answer is bound to the firm's AI-tool log entry for the filing. An initial denial of AI use that is later contradicted by the firm's own show-cause response or AI-tool log is treated by disciplinary tribunals as an independent duty-of-candor violation under Rule 3.3 separate from the underlying citation-fabrication conduct, and supports an outcome up to and including license suspension on a single first-instance disciplinary referral. The Nebraska Supreme Court's April 15–16, 2026 order in *In re Discipline of Lake* is the anchor authority for this rule and is summarized in `knowledge-base/best-practices/ai-hallucination-sanctions-2026.md`. The certification checklist's "AI-tool log" line is the operational hinge for this rule — the log entry must exist before the certification can be signed, must accurately reflect each AI tool used and the stage of drafting at which it was used, and must be retrievable on direct judicial inquiry without delay.
- Rule — *verify the proposition, not just the existence*. A citation can be GREEN on existence (the case is real, the reporter and year are correct, the quotation appears verbatim) and still be defective because the authority does not support the proposition the draft attaches to it. The Ninth Circuit's June 3, 2026 opinion in *LNU v. Blanche* (summarized in `knowledge-base/best-practices/ai-hallucination-sanctions-2026.md`) separates "fabrications" (nonexistent authority or quotations) from "inaccuracies" (real authority cited for an unsupported proposition) and holds that inaccuracies "may prove more dangerous to our profession in the long run" because they survive facial checks and require "close analysis of cited sources" to detect — and that a competent attorney "must do more than … check that the citations provided by the AI are real and the subject matter roughly on point … [a competent attorney] must also *read* and *reason*." Accordingly: never mark an item GREEN solely because the authority exists and the quotation is verbatim. Any item cited for a specific holding, rule statement, or multi-factor test must be YELLOW or higher with a verification task that confirms *holding-to-proposition alignment*, not merely existence. The verification task for such items must specifically test for the three inaccuracy modes the opinion enumerates: (a) misunderstanding the holding; (b) presenting a rejected party argument, dissent, or dicta as the court's holding; and (c) failing to respect the hierarchy of authorities (e.g., a trial-court or out-of-circuit decision cited as controlling). These map to Characterization-error (category C) and Statutory/rule-text (category D) in the Expanded Defect Taxonomy and are the harder-to-catch, higher-priority defects — not residual checks after fabrication and verbatim-quote checks are done.
...
- `firm.ethics.truthful_ai_disclosure_on_direct_inquiry` — non-overridable boolean codifying the truthful-disclosure-on-direct-inquiry rule documented in Output Requirements; the skill treats this as a hard rule even if absent from `config.yml`. The certification checklist's AI-tool-log line is the operational hinge — when the key is true (the default and the non-overridable posture), the certification cannot be completed without an AI-tool log entry that the signing attorney has reviewed and is prepared to disclose to the tribunal on direct inquiry. The codification matches the Nebraska Supreme Court's April 15–16, 2026 *Lake* order, in which an initial denial of AI use at oral argument — later contradicted by the attorney's show-cause response — supported a license-suspension order independent of the underlying citation-fabrication conduct
